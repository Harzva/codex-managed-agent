...
function launchCodexExecResume(threadId, prompt, cwd, reason = "manual", options = {}) {
  const probe = childProcess.spawnSync("codex", ["--version"], {
    encoding: "utf8",
    timeout: 2500,
  });
  if (probe.error) {
  ...
  if (probe.status !== 0) {
    throw new Error((probe.stderr || probe.stdout || "codex CLI is not available").trim());
  }
  ...
  if (!child.pid) {
    throw new Error("codex exec resume did not start a background process");
  }
  child.unref();
  setTimeout(() => {
    try {
  ...
  return { pid: child.pid, logPath, model };
}

async function sendPromptToThread(panel, threadId, prompt, options = {}) {
  if (!threadId) return;
  const nextPrompt = String(prompt || "").trim();
  if (!nextPrompt) {
  ...
  try {
    const launched = launchCodexExecResume(threadId, nextPrompt, cwd, options.reason || "manual", options);
    const queuedAt = new Date().toISOString();
    panel.optimisticQueuedPrompts[threadId] = {
      threadId,
      prompt: nextPrompt,
      queuedAt,
      state: "queued",
    ...
    delete panel.optimisticQueuedPrompts[threadId];
    panel.postMessage({
      type: "promptQueueFailed",
      threadId,
      prompt: nextPrompt,
      message: detail,
    });
